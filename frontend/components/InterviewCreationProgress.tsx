...
interface InterviewAnalysisStage {
  id: string;
  title: string;
  subtitle: string;
  icon: string;
...
const INTERVIEW_ANALYSIS_STAGES: InterviewAnalysisStage[] = [
  {
    id: 'fetching',
  ...
  },
  {
    ...
    subtitle: 'Configuring Niamh Morissey agent for your role',
    icon: 'person-outline',
    duration: 2200,
  },
  {
...
interface InterviewCreationProgressProps {
  onComplete: () => void;
}

const InterviewCreationProgress: React.FC<InterviewCreationProgressProps> = ({ onComplete }) => {
  ...
  const startProgressAnimation = () => {
    ...
    const animateStage = () => {
      if (stageIndex >= INTERVIEW_ANALYSIS_STAGES.length) {
      ...
      const stage = INTERVIEW_ANALYSIS_STAGES[stageIndex];
      const stageProgress = (stageIndex + 1) / INTERVIEW_ANALYSIS_STAGES.length;
  ...
  const currentStage = INTERVIEW_ANALYSIS_STAGES[currentStageIndex];
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Design System Accent Streaks for interview flow */}
      <LinearGradient
        colors={[
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        ...
        <View style={styles.progressContainer}>
          ...
          <Svg width={240} height={240} style={styles.svg}>
            <Defs>
              <SvgLinearGradient id="interviewProgressGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <Circle
              ...
              cy="120"
              r="100"
              stroke="url(#interviewProgressGradient)"
              strokeWidth="12"
              fill="none"
              strokeLinecap="round"
        ...
        <View style={styles.stageInfoContainer}>
          <Animated.View style={[styles.stageInfo, { opacity: stageTextOpacity }]}>
            ...
            <Text style={styles.stageSubtitle} numberOfLines={2} ellipsizeMode="tail">
              {isComplete ? 'Your personalized interview is ready' : currentStage?.subtitle}
            </Text>
          </Animated.View>
        </View>
...
export default InterviewCreationProgress;